...
export const OpenAICompatibleProvider = ({
  ...
  useEffect(() => {
    // Fetch models on mount if both base URL and API key are already set
        ...
        debounceTimerRef.current = setTimeout(() => {
          ModelsServiceClient.refreshOpenAiModels(
            ...
            .then((response) => {
              ...
              if (models.length === 0) {
                setFetchError(
                  "No models returned. Check that the base URL points to a valid OpenAI-compatible endpoint.",
                );
              }
            })
            .catch((error) => {
              ...
              setFetchError(
                error?.message
                  ...
                  : "Failed to fetch models. Check the base URL and API key.",
              );
            })
            .finally(() => {
                ...
                onChange={(e: any) => {
                  ...
                  if (value) {
                    handleModeFieldChange(
                      {
                        plan: "planModeOpenAiModelId",
                        act: "actModeOpenAiModelId",
                      },
                      value,
                      currentMode,
                    );
